...
fn xml_item_title_url(an_item: Children) -> Option<(EpisodeTitle, EpisodeUrl, EpisodeLength)> {
    let mut title_name = "";
    let mut the_url: &str = "";
    ...
    if the_url == "" {
        let url_string = xml_search_image_tag(the_text);
    ...
    for node in rss_doc.descendants() {
        if node.is_element() {
            ...
            if tag_name == "item".into() {
                ...
                let the_kids = the_children.into_iter();
                let title_and_url = xml_item_title_url(the_kids);
...
pub fn xml_clean_titles_urls(
    titles_and_urls: Vec<EpisodeMetadataTuple>,
) -> Vec<EpisodeMetadataTuple> {
...
fn xml_search_image_tag(the_text: String) -> String {
