...
pub fn name_hover(the_app: &mut app_state::DownApp, hover_event: MouseEvent) {
    let column = hover_event.column;
    let row = hover_event.row;
    if area_rects::point_in_rect(column, row, areas_consts::NEW_NAME_AREA) {
        the_app.hover_element = app_state::HOVER_NEW_NAME.to_string();
    }
}

pub fn clicked_name(the_app: &mut app_state::DownApp, the_click: MouseEvent) {
...
pub fn render_name(
    ...
    if wait_color == Color::Reset {
        if is_edit || has_chars || the_app.hover_element == app_state::HOVER_NEW_NAME {
