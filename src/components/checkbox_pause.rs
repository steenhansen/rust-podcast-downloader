...
pub fn pause_show(console_frame: &mut Frame, the_app: &mut state_app::DownApp) {
    if the_app.selected_podcast != "" {
        ...
        let pause_area = const_areas::PAUSE_AREA;
...
pub fn pause_hover(the_app: &mut state_app::DownApp, hover_event: MouseEvent) {
    ...
    if misc_ui::rect_point_in(column, row, const_areas::PAUSE_AREA) {
...
pub fn pause_render(
    ...
    let mut the_style = Style::default().fg(const_colors::PAUSE_DEAD);
    if g_active::active_downloading() > 0 {
        if the_app.hover_element == state_app::HOVER_PAUSE {
            the_style = Style::default().fg(const_colors::PAUSE_HOVER);
        } else {
            the_style = Style::default().fg(const_colors::PAUSE_READY);
        }
    }
