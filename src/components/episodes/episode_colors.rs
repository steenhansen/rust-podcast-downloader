...
pub fn colors_start_down_done(
    ...
    if num_bytes == consts_globals::DOWN_BYTES_FINISHED {
...
pub fn colors_possible_or_waiting(
    ...
    if the_waiting.contains_key(full_epi_name) {
        let indented_waiting = consts_globals::WAITING_FINISHED_INDENT.to_owned() + episode_name;
        let waiting_file = colors_waiting_epi(&indented_waiting, consts_globals::ON_WAITLIST);
...
pub fn colors_hover_epi(episode_name: &str) -> Span<'static> {
    let waiting_file = Span::styled(
        ...
        Style::default()
            .fg(consts_globals::AN_EPISODE_TEXT_HOVER)
            .bg(consts_globals::AN_EPISODE_BACK_HOVER),
    );
    waiting_file
}
...
pub fn colors_old_epi(episode_name: &str, wait_color: Color) -> Span<'static> {
    let old_have_color = match wait_color {
        Color::Reset => consts_globals::OLD_LOCAL_EPISODE,
        _ => wait_color,
    };
    let indented_old = consts_globals::WAITING_FINISHED_INDENT.to_owned() + episode_name;
...
pub fn colors_finished_epi(episode_name: &str, wait_color: Color) -> Span<'static> {
    let finished_color = match wait_color {
        Color::Reset => consts_globals::JUST_GOT_EPISODE,
        _ => wait_color,
    };
    let just_finished = consts_globals::WAITING_FINISHED_INDENT.to_owned() + episode_name;
...
pub fn colors_start_epi(episode_name: &str, wait_color: Color) -> Span<'static> {
    let start_text_color = match wait_color {
        Color::Reset => consts_globals::START_EPISODE_FG,
        _ => wait_color,
    };
    let start_back_color = match wait_color {
        Color::Reset => consts_globals::START_EPISODE_BG,
        _ => consts_globals::BLACK_WAIT,
    };
    let new_f_pos = consts_globals::DOWN_BYTES_START_SHOW.to_owned() + " - " + episode_name;
...
pub fn colors_continue_epi(
    ...
    let continue_color = match wait_color {
        Color::Reset => consts_globals::ACTIVE_EPISODE,
        _ => wait_color,
    };
...
pub fn colors_waiting_epi(episode_name: &str, wait_color: Color) -> Span<'static> {
    let waiting_color = match wait_color {
        Color::Reset => consts_globals::WAITING_EPISODE,
        _ => wait_color,
    };
...
pub fn colors_possible_epi(episode_name: &str, wait_color: Color) -> Span<'static> {
    let possible_color = match wait_color {
        Color::Reset => consts_globals::CAN_DOWNLOAD_EPISODE,
        _ => wait_color,
    };
