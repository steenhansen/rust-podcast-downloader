...
pub fn every_show(console_frame: &mut Frame, the_app: &mut state_app::DownApp) {
    every_episode_rendered(
        console_frame,
        consts_areas::EVERY_EPISODE_AREA,
        the_app,
...
pub fn every_episode_rendered(
    ...
    let mut every_text_color = consts_globals::BTN_EVERY_TEXT_DEAD;
    let mut every_background_color = consts_globals::BTN_EVERY_BACK_DEAD;

    if the_app.ui_state == consts_types::UiState::State401DownloadPaused {
        every_text_color = consts_globals::PAUSE_TEXT_COLOR;
        every_background_color = consts_globals::PAUSE_BACK_COLOR;
    } else if every_active(the_app) {
        if the_app.hover_element == state_app::HOVER_BTN_EVERY {
            every_text_color = consts_globals::BTN_EVERY_TEXT_HOVER;
            every_background_color = consts_globals::BTN_EVERY_BACK_HOVER;
        } else {
            every_text_color = consts_globals::BTN_EVERY_TEXT_READY;
            every_background_color = consts_globals::BTN_EVERY_BACK_READY;
        }
    }
