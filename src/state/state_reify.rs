...
pub fn reify_type(the_app: &mut state_app::DownApp) {
    if the_app.ui_state == const_types::UiState::State101ReadingRss {
        let podcast_name = the_app.selected_podcast.clone();
        if podcast_name != "" {
            the_app.ui_state = const_types::UiState::State102ShowWaiting;
        } else {
            the_app.ui_state = const_types::UiState::StateNoFocus; // clicked on empty space
        }
    ...
    } else if the_app.ui_state == const_types::UiState::State202SureEveryEpisode {
    ...
    } else if the_app.ui_state == const_types::UiState::State601KillingDownloads {
